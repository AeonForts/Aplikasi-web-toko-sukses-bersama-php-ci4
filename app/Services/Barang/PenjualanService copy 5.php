...
class PenjualanService
{
    ...
    //     $total_meity = (float)$pembelian['total_meity'];
    //     $current_terkumpul = (float)($latestMeityRecord['terkumpul'] ?? 0);
    //     $new_terkumpul = $current_terkumpul + (float)$totalHargaModal;
    ...
    //         case 1:
    //             $updateData['current_piutang'] = (float)($latestMeityRecord['current_piutang'] ?? 0) + (float)$totalHargaModal;
    //             break;
    //         case 2:
    //             $updateData['current_transfer'] = (float)($latestMeityRecord['current_transfer'] ?? 0) + (float)$totalHargaModal;
    //             break;
    //     }

    //     if (bccomp($new_terkumpul, $total_meity, 8) > 0) {
    ...
    //             $totalAmount = (float)$latestDetailPenjualan['jumlah_keluar'];
    //             // Calculate exact amounts without rounding
    //             $mainAmount = ($total_meity - $current_terkumpul) / ($totalHargaModal / $totalAmount);
    //             $excessAmount = $totalAmount - $mainAmount;

    //             // Update original detail penjualan with exact amount
    ...
    //                 $newExcessModal = $excessAmount * (float)$nextDetailPembelian['harga_modal_barang'];
    //                 $excessPaymentAmount = $excessAmount * (float)$latestDetailPenjualan['harga_jual'];
    ...
    //                 // Update original payment with exact amount
    //                 $this->paymentService->updatePayment($latestDetailPenjualan['id_payment'], [
    //                     'jumlah' => $mainAmount * (float)$latestDetailPenjualan['harga_jual']
    //                 ]);

    //                 // Create new detail penjualan for excess with exact amount
    //                 $excessDetailData = [
    ...
    //                 // Handle next meity record with exact values
    ...
    //                 $nextMeityUpdate = [
    //                     'terkumpul' => (float)($nextMeityRecord['terkumpul'] ?? 0) + $newExcessModal
    //                 ];

    //                 switch ($paymentMethod) {
    //                     case 1:
    //                         $nextMeityUpdate['current_piutang'] = (float)($nextMeityRecord['current_piutang'] ?? 0) + $newExcessModal;
    //                         break;
    //                     case 2:
    //                         $nextMeityUpdate['current_transfer'] = (float)($nextMeityRecord['current_transfer'] ?? 0) + $newExcessModal;
    //                         break;
    //                 }

    //                 $nextMeityUpdate['status'] = $this->calculateStatus(
    //                     (float)($nextMeityRecord['total_meity'] ?? $total_meity),
        ...
        if (bccomp($new_terkumpul, $total_meity, 8) > 0) {
            ...
            if ($latestDetailPenjualan) {
                ...
                try {
                    ...
                    // Get the total_meity for the next pembelian
                    $nextPembelian = $this->pembelianModel->find($nextPembelianId);
                    $nextTotalMeity = (float)($nextPembelian['total_meity'] ?? 0);
                    ...
                    // Calculate status for the next meity based on its own total_meity
                    $nextMeityUpdate['status'] = $this->calculateStatusForNext(
                        $nextTotalMeity,
                        $nextMeityUpdate['terkumpul']
                    );
    ...
    // New helper method specifically for calculating status of next meity after split
    private function calculateStatusForNext($total_meity, $terkumpul)
    {
        if ($terkumpul == 0) {
            return 0; // Belum Lunas
        } elseif ($terkumpul >= $total_meity) {
            return 1; // Only set to Menunggu Konfirmasi if actually meets requirement
        }
        
        return 0; // Keep as Belum Lunas for partial payments
    }
    ...
    {
        // Fetch all detail penjualan records for this penjualan and type, excluding status 0
        $detailRecords = $this->detailPenjualanModel
            ->where('id_penjualan', $id_penjualan)
            ->where('id_tipe', $id_tipe)
            ->where('status !=', 0) // Exclude records with status 0 (belum lunas)
            ->findAll();
    
        if (empty($detailRecords)) {
            log_message('error', "No valid detail penjualan records found for id_penjualan: $id_penjualan and id_tipe: $id_tipe");
            return false;
        }
        ...
        // Get total_barang_keluar including status 0 records
        $allDetailRecords = $this->detailPenjualanModel
            ->where('id_penjualan', $id_penjualan)
            ->where('id_tipe', $id_tipe)
            ->findAll();
    
        $totalAllBarangKeluar = 0;
        foreach ($allDetailRecords as $record) {
            $totalAllBarangKeluar += $record['jumlah_keluar'];
        }
        ...
        $updateData = [
            ...
            'total_barang_keluar' => $totalAllBarangKeluar // Use total including status 0
        ];
