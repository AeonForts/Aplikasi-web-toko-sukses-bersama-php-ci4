...
$(document).ready(function() {
  ...
  $('#id_tipe').on('change', function() {
    ...
    if (selectedId) {
        $.ajax({
            url: '<?= base_url('petugas/penjualan/getTipeBarang') ?>',
            type: 'GET',
            dataType: 'json',
...
`);
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '<?= base_url('petugas/penjualan/save'); ?>',
                type: 'POST',
                dataType: 'json',
                data: { 
                    cart: cartData,
...
$('#printReceiptBtn').on('click', function() {
    ...
    if (window.lastTransactionResponse && window.lastTransactionResponse.invoiceId) {
        ...
        $.ajax({
            url: '<?= base_url('petugas/penjualan/print-invoice/') ?>' + invoiceId,
            type: 'GET',
            dataType: 'json',
...
$('#cancelPrintBtn').on('click', function() {
    ...
    $('#printReceiptModal').modal('hide');
        ...
        function confirmPayment(id_pembelian) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: '<?= base_url('petugas/pembelian/confirmPayment') ?>',
                        type: 'POST',
    ...
    function initializePenjualanChart(chartType = 'total_barang_keluar', idTipe = '') {
        $.ajax({
            url: '<?= base_url('petugas/penjualan/chart-data'); ?>',
            method: 'GET',
            data: {
    ...
    function populateTipeBarangDropdown() {
        $.ajax({
            url: '<?= base_url('petugas/barang/getTipeBarang'); ?>',
            method: 'GET',
            dataType: 'json',
