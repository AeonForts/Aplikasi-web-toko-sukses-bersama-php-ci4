        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="form-group position-relative">
                        ...
                        <input type="text" class="form-control" id="nama_supplier" name="nama_supplier" autocomplete="off" placeholder="Cari Supplier">
                    </div>
                    ...
                    <div class="form-group">
                        <label for="harga_modal_barang">Harga Modal Barang
                        <i class="fas fa-exclamation-circle text-info ml-1" 
                                        data-toggle="tooltip" 
                                        data-placement="right" 
                                        title="Maksimal Angka Decimal 8">
                        </i>    
                        </label>
                        <input type="text" name="harga_modal_barang" class="form-control" id="harga_modal_barang" required>
                    </div>
                    <div class="form-group">
                        <label for="total_meity">Estimasi Total Meity</label>
                        <input type="text" name="total_meity" class="form-control" id="total_meity" readonly>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="col-md-6">
                            <div class="card">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="form-group">
                                        <label for="edit_harga_modal_barang">Harga Modal
                                        <i class="fas fa-exclamation-circle text-info ml-1" 
                                        data-toggle="tooltip" 
                                        data-placement="right" 
                                        title="Masukkan harga jual barang dalam format angka">
                                        </i>    
                                        </label>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="text" class="form-control" id="edit_total_meity" name="total_meity" required >
                                        </div>
                                    </div>
...
$(document).ready(function() {
    ...
    // Add event listeners to calculate total Meity on input
    $('#barang_masuk, #harga_modal_barang').on('input', function() {
        calculateEstimatedTotal();
    });

    // Function to calculate estimated total
    function calculateEstimatedTotal() {
        var barang_masuk = $('#barang_masuk').val() || 0;
        var harga_modal_barang = $('#harga_modal_barang').val() || 0;
        ...
        // Update the read-only field with calculated value
        $('#total_meity').val(total_meity);
    }
    ...
    $('#editPembelianModal').on('hidden.bs.modal', function () {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $('#nama_supplier').on('input', function() {
                var input = $(this).val().toLowerCase();
                var inputWidth = $(this).outerWidth();
                var resultsContainer = $('#supplier-results');

                // Create results container if it doesn't exist
                if (resultsContainer.length === 0) {
                    $(this).after('<div id="supplier-results" class="list-group position-absolute"></div>');
                    resultsContainer = $('#supplier-results');
                }
                ...
                if (input.length >= 2) {
                    var matchingSuppliers = window.suppliers.filter(function(supplier) {
                        return supplier.nama_supplier.toLowerCase().includes(input);
                    });

                    // Populate results
                    matchingSuppliers.forEach(function(supplier) {
                        resultsContainer.append(
                            `<a href="#" class="list-group-item list-group-item-action supplier-option" 
            ...
            $(document).on('click', '.supplier-option', function(e) {
                ...
                $('#nama_supplier').val(selectedSupplier);
                $('#supplier-results').hide();
            });

            // Hide results when clicking outside
            $(document).on('click', function(e) {
                if (!$(e.target).closest('#nama_supplier, #supplier-results').length) {
                    $('#supplier-results').hide();
                }
            });
        },
    ...
    $('#nama_supplier').on('input', function() {
        ...
        if (input) {
            $.ajax({
                ...
                dataType: 'json',
                success: function(data) {
                    var matchingSuppliers = data.filter(item => item.nama_supplier.toLowerCase().includes(input.toLowerCase()));
                    if (matchingSuppliers.length > 0) {
                        // Create a dropdown list of matching suppliers
                        var dropdownList = '';
                        matchingSuppliers.forEach(supplier => {
                            dropdownList += `<option value="${supplier.nama_supplier}">${supplier.nama_supplier}</option>`;
                        });
                        $('#nama_supplier').html(dropdownList);
                    } else {
                        // Handle no matching suppliers
        ...
        success: function(response) {
            if (response.status === 'success') {
                Swal.fire({
                    ...
                    window.location.href = '<?= base_url('admin/pembelian'); ?>'; // Change this to your desired URL
                });
            } else {
                Swal.fire({
                    icon: 'error',
