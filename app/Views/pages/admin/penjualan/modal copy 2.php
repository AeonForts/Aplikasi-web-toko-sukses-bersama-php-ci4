...
$(document).ready(function() {
    ...
    $('#tipe_unit').on('change', function() {
        ...
        // Validate inputs
        if (hargaJual <= 0 || standarJumlah <= 0) {
            $('#jumlah_keluar').val('0.00');
            return;
        }

        // Advanced calculation method
        // 1. Calculate maximum whole units
        const maxUnits = Math.floor(uang / hargaJual);

        // 2. Calculate total price for max units
        const totalPrice = maxUnits * hargaJual;

        // 3. Calculate outgoing quantity
        const barangKeluar = maxUnits * standarJumlah;

        // 4. Calculate remaining money (change)
        const kembalian = uang - totalPrice;

        // Update fields
        $('#jumlah_keluar').val(barangKeluar.toFixed(2));
        
        // Optional: You might want to add a hidden field for kembalian
    ...
    // Attach calculation to relevant input events
    $(document).ready(function() {
        // Recalculate when key inputs change
        $('#jumlah, #harga_jual, #standar_jumlah_barang').on('input', calculatePurchase);

        // Additional event listeners if needed
        $('#id_tipe, #tipe_unit').on('change', calculatePurchase);
    });
    // Add item to cart
    ...
    if (window.lastTransactionResponse) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.status === 'success') {
                    // Create a print-specific function
