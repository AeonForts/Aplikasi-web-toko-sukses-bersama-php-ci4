...
use App\Models\{StockBarangModel,BarangModel,UnitBarangModel,ViewStockModel};
...
class BarangController extends BaseController
{
    ...
    public function listStock()
    {
        ...
        return view('pages/petugas/barang/list_stock_barang', $data);
    }
    ...
    public function list($view = 'list')
    {
        ...
        $viewPath = 'pages/petugas/barang/' . ($view === 'list_barang' ? 'list_barang' : 'list');
    ...
    public function getDatatables()
    {
        ...
        // Base query with join to get barang details
        $query = $this->viewStockModel
            ->select('view_stock_with_sisa_per_unit.*, tbl_tipe_barang.jenis_barang, tbl_tipe_barang.satuan_dasar')
            ->join('tbl_tipe_barang', 'view_stock_with_sisa_per_unit.id_tipe = tbl_tipe_barang.id_tipe')
            ->orderBy('tgl_stock', 'DESC'); // Add this line to ensure DESC ordering
        ...
        $response = [
            ...
            'data' => array_map(function($item) {
                return [
                    ...
                    'tgl_stock' => date('d-m-Y', strtotime($item['tgl_stock'])), // Display format
                    'tgl_stock_sort' => $item['tgl_stock'], // Original date for sorting
    ...
    public function save()
    {
        ...
        $standar_jumlah_barang = $this->request->getPost('standar_jumlah_barang') ?? 0;
        $standar_harga_jual = $this->request->getPost('standar_harga_jual') ?? 0;
    ...
    public function update()
    {
        ...
        $satuan_dasar = $this->request->getPost('satuan_dasar');
        $tipe_unit = $this->request->getPost('tipe_unit');
        $standar_jumlah_barang = $this->request->getPost('standar_jumlah_barang');
