...
class PenjualanController extends BaseController
{
    ...
    {
        $data = [
            'vw_detail_penjualan' => $this->viewDetailPenjualanModel->getDetailByPenjualanId($id),
            'tipeBarangList' => $this->barangModel->findAll()
        ];
            ...
            if ($this->request->getPost('cart')) {
                ...
                foreach ($processedItems as $item) {
                    ...
                    $stockInput = [
                        ...
                        'tgl_stock' => $tgl_penjualan,
                        'barang_masuk' => 0,
                        'barang_keluar' => $input['jumlah_keluar']
                    ];
                    $this->stockService->updateStockWithSpecificDate($stockInput);
...
'storeAddress' => 'Jl. Rambutan Raya No. 42<br>RT.06/RW.06 Baranangsiang<br>Kec. Bogor Timur, Kota Bogor<br>Jawa Barat 16143',                    'phone' => '082175671616',
    ...
    public function editDetailPenjualan()
    {
        ...
        try {
            $idDetailPenjualan = $this->request->getPost('id_detail_penjualan');
            $detailPenjualan = $this->viewDetailPenjualanModel->getDetailPenjualanWithMethod($idDetailPenjualan);
            ...
            $this->db->transRollback();
            log_message('error', 'Get Detail Penjualan Error: ' . $e->getMessage());
            return $this->response->setStatusCode(500)->setJSON([
                'status' => 'error',
                'message' => $e->getMessage()
            ]);
    ...
    public function updateDetailPenjualan()
    {
        ...
        try {
            $idDetailPenjualan = $this->request->getPost('id_detail_penjualan');
            $newMethodId = $this->request->getPost('id_method');
            $idPayment = $this->request->getPost('id_payment');
            $newStatus = $this->request->getPost('status');
            
            // Log the received data
            log_message('debug', 'Update params: idDetailPenjualan=' . $idDetailPenjualan . 
            ...
            $detailPenjualan = $this->db->table('tbl_detail_penjualan')
                ->select('tbl_detail_penjualan.*, tdp.harga_modal_barang, tp.id_method as current_method_id')
                ->join('tbl_detail_pembelian tdp', 'tbl_detail_penjualan.id_pembelian = tdp.id_pembelian', 'left')
                ->join('tbl_payment tp', 'tbl_detail_penjualan.id_payment = tp.id_payment', 'left')
                ->where('id_detail_penjualan', $idDetailPenjualan)
                ->get()
            ...
            // If method hasn't changed, no need to update meity records
            if ($detailPenjualan['current_method_id'] == $newMethodId) {
                // Just update the payment in detail_penjualan
                ...
                return $this->response->setJSON([
                    'status' => 'success',
                    'message' => 'Payment method updated successfully (no change in method type)'
                ]);
            }
    
            // Calculate the modal value for this detail
            $totalHargaModal = $detailPenjualan['jumlah_keluar'] * $detailPenjualan['harga_modal_barang'];
            $totalHargaJual = $detailPenjualan['jumlah_keluar'] * $detailPenjualan['harga_jual'];
    
            $newMethod = $this->db->table('tbl_payment')
                ->select('id_method')
                ->where('id_payment', $idPayment)
                ->get()
                ->getRowArray();
            
            // Ensure we got a valid method ID
            if (!$newMethod) {
                throw new Exception("Invalid payment ID: No corresponding method found.");
            }

            // Find the meity record for this specific pembelian and tipe
            $meityRecord = $this->db->table('tbl_meity')
                ->where('id_tipe', $detailPenjualan['id_tipe'])
                ->where('id_pembelian', $detailPenjualan['id_pembelian'])
                ->get()
                ->getRowArray();
            ...
            if ($meityRecord) {
                ...
                // First handle removal from old method - only for the specific transaction amount
                switch ($detailPenjualan['current_method_id']) {
                    case 1: // Piutang
                        $newCurrentPiutang = max(0, $meityRecord['current_piutang'] - $totalHargaModal);
                        $meityUpdateData['current_piutang'] = $newCurrentPiutang;
                        break;
                    case 2: // Transfer
                        $newCurrentTransfer = max(0, $meityRecord['current_transfer'] - $totalHargaModal);
                        $meityUpdateData['current_transfer'] = $newCurrentTransfer;
                        break;
                    case 3: // Cash - no action needed
                        break;
                }
    
                // Then add to new method (only if not cash)
                if ($newMethodId != 3) { // Skip for cash method
                    switch ($newMethodId) {
                        case 1: // Piutang
                            if ($newStatus != 1) { // Skip adding new current piutang if status is 1
                                $newCurrentPiutang = isset($meityUpdateData['current_piutang']) 
                                    ? $meityUpdateData['current_piutang'] + $totalHargaModal 
                                    : $meityRecord['current_piutang'] + $totalHargaModal;
                                $meityUpdateData['current_piutang'] = $newCurrentPiutang;
                            }
                            break;
                        case 2: // Transfer
                            $newCurrentTransfer = isset($meityUpdateData['current_transfer']) 
                                ? $meityUpdateData['current_transfer'] + $totalHargaModal 
                                : $meityRecord['current_transfer'] + $totalHargaModal;
                            $meityUpdateData['current_transfer'] = $newCurrentTransfer;
                            break;
                    }
                }
    
                // If we have changes to make to the meity record
                if (!empty($meityUpdateData)) {
                    // Log the update data
                    log_message('debug', 'Meity update for method change: ' . json_encode($meityUpdateData));
    
                    // Update meity record
                    $this->db->table('tbl_meity')
                        ->where('id_meity', $meityRecord['id_meity'])
                        ->update($meityUpdateData);
                }
            }

            if($newMethodId == 1 && $newStatus == 0){
            ...
            // Update the payment in detail_penjualan
            $this->db->table('tbl_detail_penjualan')
                ->where('id_detail_penjualan', $idDetailPenjualan)
                ->update([
                        'id_payment' => $idPayment,
        ...
        try {
            ...
            $latestMeityRecord = $this->db->table('tbl_meity')
                ...
                if ($latestMeityRecord) {
                    ...
                    $meityUpdateData['status'] = 0;  // Explicitly set status to 0
                    
                    // Log final update data
                    log_message('debug', 'Final Meity update: ' . json_encode($meityUpdateData));
